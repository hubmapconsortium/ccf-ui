...
import { OntologyTreeNode } from 'ccf-database';  //TODO: change to CellTypeTreeNode
import { get, sortBy } from 'lodash';
import { GoogleAnalyticsService } from 'ngx-google-analytics';
import { Observable } from 'rxjs';
import { filter, map, startWith, switchMap } from 'rxjs/operators';

import { OntologySearchService, SearchResult } from '../../../core/services/ontology-search/ontology-search.service';  //TODO: change to CellTypeSearchService


/**
 * Component for searching the Cell Type nodes.
 */
@Component({
  selector: 'ccf-cell-type-search',
  templateUrl: './cell-type-search.component.html',
  styleUrls: ['./cell-type-search.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class CellTypeSearchComponent implements OnInit {
  /**
   * Output event-emitter which emits the id of the CellTypeTreeNode whose label was
  ...
  /**
   * Creates an instance of cell type search component.
   *
   * @param cellTypeService instance of searchService which provides all the search functionality
   * @param ga Analytics service
   */
  constructor(public cellTypeService: OntologySearchService, private readonly ga: GoogleAnalyticsService) { }

  /**
   * on-init lifecycle hook for this component -
   * gets the searched value from the view, sends it to the filter function in the CellTypeService,
  ...
  ngOnInit(): void {
    ...
    this.filteredResults$ = valueChanges.pipe(
      ...
      startWith(''),
      switchMap(value => this.cellTypeService.filter(value)),
  ...
  onSelect(event: MatAutocompleteSelectedEvent): void {
    ...
    if (node) {
      this.ga.event('search', 'cell_type_search', node.id);
