...
/**
 * Represents a expandable tree of cell types.
 */
@Component({
  selector: 'ccf-cell-type-tree',
  templateUrl: './cell-type-tree.component.html',
  styleUrls: ['./cell-type-tree.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class CellTypeTreeComponent implements OnInit, OnChanges {
  /**
   * Input of cell type filter, used for changing the cell type selections
  ...
  /**
   * List of nodes in the cell type tree
   */
  ...
  /**
   * Creates an instance of cell type tree component.
   *
  ...
  ngOnChanges(changes: SimpleChanges): void{
    if (changes.cellTypeFilter) {
      const cellTypeFilter: string[] = changes.cellTypeFilter.currentValue as string[];
      if (cellTypeFilter?.length >= 0) {
        this.selectByIDs(cellTypeFilter);
      } else {
  ...
  selectByIDs(ids: string[]): void {
    ...
    if (selectedNodes?.length > 0) {
      ...
      this.ga.event('nodes_selected_by_ids', 'cell_type_tree', selectedNodes.map(node => node.label).join(','));
  ...
  select(ctrlKey: boolean, node: FlatNode | undefined, emit: boolean, select: boolean): void {
    ...
    if (node === undefined) {
      this.selectedNodes = [];
      this.ga.event('nodes_unselected', 'cell_type_tree');
      return;
    }
    ...
    this.ga.event('nodes_selected', 'cell_type_tree', this.selectedNodes.map(n => n.label).join(','));
  ...
  updateOpacity(node: FlatNode, value: number | undefined): void {
    node.opacity = value;
    this.ga.event('opacity_update', 'cell_type_tree', node.label, value);
  ...
  resetNode(node: FlatNode): void {
    node.opacity = 20;
    node.visible = true;
    this.ga.event('node_reset', 'cell_type_tree', node.label);
    this.nodeChanged.emit(node);
  }
  ...
  toggleVisibility(node: FlatNode): void {
    ...
    this.ga.event('visibility_update', 'cell_type_tree', node.label, +node.visible);
    this.nodeChanged.emit(node);
  }

  /**
   * Used to properly set the position of the slider popup on the cell type tree
   *
   * @param level Current level of a node in the cell type tree
